...
def model_cv(x_train, y_train):
    ...
    rf_model = RandomForestClassifier(n_estimators=1000, random_state=10, n_jobs=-1, oob_score=True)
    param_grid = {
...
def main():
    ...
    for i, (dev_index, val_index) in enumerate(kf.split(train_all, y_train_all)):
        ...
        model = RandomForestClassifier(n_estimators=model_params['n_estimators'], random_state=10, n_jobs=-1,
