...
from get_datasets import load_new_datsets
...
def main():
    ...
    train, test = load_new_datsets()

    y_train_all = train['orderType']
    id_test = test['userid']

    train.drop(['userid', 'gender', 'province', 'age', 'orderType', 'orderTime'], axis=1, inplace=True)
    test.drop(['userid', 'gender', 'province', 'age', 'orderTime'], axis=1, inplace=True)
    ...
    test_size = 1.0 * test.shape[0] / (test.shape[0] + train.shape[0])
    print('valid split ratio: ', test_size)
    X_train, X_valid, y_train, y_valid = train_test_split(train, y_train_all, test_size=test_size, random_state=42)
